	...
	switch (loopResult.path) {
		...
		case 'titles.job':
			...
			if (acceptAllResult.isSuccess && acceptAllResult.path === 'jobs.acceptAll.enabled') {
				...
				await macroService.pollPattern(patterns.jobs.acceptAll.enabled, { doClick: true, clickPattern: [patterns.branchEvent.availableNow, patterns.branchEvent.playLater, patterns.prompt.playerRankUp], predicatePattern: patterns.jobs.prompt.ok });
				await macroService.pollPattern(patterns.jobs.prompt.ok, { doClick: true, clickPattern: [patterns.branchEvent.availableNow, patterns.branchEvent.playLater, patterns.prompt.playerRankUp], predicatePattern: patterns.titles.job });
