...
while (state.isRunning && !done) {
	...
	switch (loopResult.path) {
		case 'lobby.everstone':
			logger.info('doGateBreakthrough undead: click adventure');
			await macroService.clickPattern(patterns.lobby.adventure);
			await sleep(500);
			break;
		case 'titles.adventure':
			logger.info('doGateBreakthrough undead: click gate breakthrough');
		...
		case 'titles.gateBreakthrough':
			logger.info('doGateBreakthrough undead: click undead gate');
			await macroService.pollPattern(patterns.gateBreakthrough.gates.undead, { doClick: true, predicatePattern: patterns.gateBreakthrough.challenge });
			await sleep(500);
			break;
		case 'gateBreakthrough.challenge':
			logger.info('doGateBreakthrough undead: click challenge');
		...
		case 'gateBreakthrough.nextStage':
			logger.info('doGateBreakthrough undead: click next stage');
		...
		case 'gateBreakthrough.retry':
			logger.info('doGateBreakthrough undead: retry');
			done = true;
