...
function sweepEventStoryHard(number) {
	...
	const settingPrefix = number === 1 ? 'sweepEventStoryHard' : 'sweepEventStoryHard2';
	const dailyPrefix = number === 1 ? 'sweepEventStoryHard' : 'sweepEventStoryHard2';
	const logPrefix = number === 1 ? 'sweepEventStoryHard' : 'sweepEventStoryHard2';
	const teamSlot = settings[settingPrefix].teamSlot.Value;

	if (daily[dailyPrefix].done.IsChecked) {
		return "Script already completed. Uncheck done to override daily flag.";
	}

	const targetEventPattern = macroService.ClonePattern(settings[settingPrefix].targetEventPattern.Value, {
		X: 80,
		Y: 215,
	...
	const targetPartPattern = macroService.ClonePattern(settings[settingPrefix].targetPartPattern.Value, {
		X: 80,
		Y: 215,
		Width: resolution.Width - 100,
		Height: 785,
		...
		switch (loopResult.Path) {
			...
			case 'event.story.enter':
				...
				if (!storyPartSwipeResult.IsSuccess) {
					throw Error(`Unable to find pattern: settings.${settingPrefix}.targetPartPattern`);
				}
				...
				if (macroService.IsRunning) {
					daily[dailyPrefix].done.IsChecked = true;
				}
				return;
		}
		sleep(1_000);
