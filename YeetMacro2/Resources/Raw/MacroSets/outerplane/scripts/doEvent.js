...
function doEvent(type) {
	...
	} else if (type === 'storyHard3') {
		return sweepEventStoryHard(3);
	} else {
...
function sweepEventStoryHard(number) {
	...
	if (daily.doEvent[dailyPrefix].IsChecked) {
		...
		switch (loopResult.Path) {
			...
			case 'event.story.enter':
				...
				if (macroService.IsRunning) {
					daily.doEvent[dailyPrefix].IsChecked = true;
				}
				return;
		}
		sleep(1_000);
	...
	while (macroService.IsRunning) {
		...
		switch (loopResult.Path) {
			...
			case 'event.story.exit':
				...
				macroService.PollPattern(patterns.event.story.rewardNotification, { DoClick: true, PredicatePattern: patterns.event.story.rewardNotification.popup, IntervalDelayMs: 4_000 });
