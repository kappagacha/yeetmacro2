        ...
        while (macroService.IsRunning)
        {
            ...
            switch (loopResult.Path)
            {
                case "lobby.everstone":
                    logger.LogInformation("doGateBreakthrough fairy: click adventure");
                    macroService.ClickPattern(patterns["lobby"]["adventure"]);
                    new System.Threading.ManualResetEvent(false).WaitOne(500);
                    break;
                case "titles.adventure":
                    logger.LogInformation("doGateBreakthrough fairy: click gate breakthrough");
                ...
                case "titles.gateBreakthrough":
                    logger.LogInformation("doGateBreakthrough fairy: click fairy gate");
                    macroService.PollPattern(patterns["gateBreakthrough"]["gates"]["fairy"], new PollPatternFindOptions() { DoClick = true, PredicatePattern = patterns["gateBreakthrough"]["challenge"] });
                    new System.Threading.ManualResetEvent(false).WaitOne(500);
                    break;
                case "gateBreakthrough.challenge":
                    logger.LogInformation("doGateBreakthrough fairy: click challenge");
                ...
                case "gateBreakthrough.nextStage":
                    logger.LogInformation("doGateBreakthrough fairy: click next stage");
                ...
                case "gateBreakthrough.retry":
                    logger.LogInformation("doGateBreakthrough fairy: retry");
