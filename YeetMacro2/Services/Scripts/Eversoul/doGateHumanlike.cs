        ...
        while (macroService.IsRunning)
        {
            ...
            switch (loopResult.Path)
            {
                case "lobby.everstone":
                    logger.LogInformation("doGateBreakthrough humanlike: click adventure");
                    macroService.ClickPattern(patterns["lobby"]["adventure"]);
                    Sleep(500);
                    break;
                case "titles.adventure":
                    logger.LogInformation("doGateBreakthrough humanlike: click gate breakthrough");
                ...
                case "titles.gateBreakthrough":
                    logger.LogInformation("doGateBreakthrough humanlike: click humanlike gate");
                    macroService.PollPattern(patterns["gateBreakthrough"]["gates"]["humanlike"], new PollPatternFindOptions() { DoClick = true, PredicatePattern = patterns["gateBreakthrough"]["challenge"] });
                    Sleep(500);
                    break;
                case "gateBreakthrough.challenge":
                    logger.LogInformation("doGateBreakthrough humanlike: click challenge");
                ...
                case "gateBreakthrough.nextStage":
                    logger.LogInformation("doGateBreakthrough humanlike: click next stage");
                ...
                case "gateBreakthrough.retry":
                    logger.LogInformation("doGateBreakthrough humanlike: retry");
